 ...
 *       require('sys').puts(compiled({name: 'Jim'}));
 *     });
 * 
...
Mu.compile = function Mu_compile(filename) {
  ...
  Mu.Parser.parse(filename, Mu.templateRoot, Mu.tempateExtension)
...
Mu.enumerable = function Mu_enumerable(context, val, fn, next) {
  ...
  if (val instanceof Array) {
    ...
    var done = function () {
      if (i++ < val.length) {
        insertProto(val[i-1], context);
        fn(val[i-1], function () {
          insertProto(val[i-1], baseProto, context);
          done();
        });
      } else {
        next();
